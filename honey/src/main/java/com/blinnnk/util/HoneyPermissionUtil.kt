...
abstract class CheckPermission(private val activity: FragmentActivity?) {

    abstract var permissionType: PermissionCategory
    open var retryTimes: Int = 10

    private fun Activity.checkPermissionThen(type: PermissionCategory, callback: () -> Unit) {
        if (verifyMultiplePermissions(type)) {
            callback()
        } else {
            requestPermissionListener(type) { hasPermission ->
    ...
    fun start(callback: () -> Unit) {
        activity?.checkPermissionThen(permissionType, callback)
    }
}
...
fun Activity.requestPermissionBy(category: PermissionCategory) {
    ActivityCompat.requestPermissions(
            this,
            arrayOf(category.getDetail().category),
            category.getDetail().requestCode
    )
}
 ...
 */
fun Activity.requestPermissionListener(
        vararg categories: PermissionCategory,
...
fun Activity.requestPermissionListener(
        categories: ArrayList<PermissionCategory>,
        callback: (Boolean) -> Unit
) {
    categories.filterNot { it.checkAppPermissionStatus() }.apply {
        if (isNotEmpty()) {
